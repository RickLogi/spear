...
Our next step is to activate our `spear-env` Python environment (Anaconda or `uv`) and launch Claude Code from this repository's root directory. At this point, we can control the UE application using natural language. For example, we can ask Claude Code,
